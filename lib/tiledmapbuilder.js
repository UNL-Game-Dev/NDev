...
Crafty.c("TiledMapBuilder", {
	...
	_callback:null,
    init: function() {
    	this._renderMethod = this.has(this.tileMapBuilderSetting.RENDER_METHOD_DOM) ?
    		...
    		layers[layer] = [];
    		 for(var idx = 0; idx < mockEntities[layer].length; idx++ ){
    			 ...
    			 if( mockEntity == 0 ){
					...
					var entity = Crafty.e( mockEntity.head ).attr({ x:mockEntity.x, y:mockEntity.y, gid:mockEntity.tilegid });
	    			if( isIsometric ){
